...
;进入主程序
        ...
        ADD     R3,R3,R1
        ADD     R1,R3,#0
loop    ADD     R0,R1,#1        ;遍历每一个点，得到最长路径
        ADD     R1,R1,#1        ;用R1控制循环次数
        ADD     R6,R6,#-1       ;保护R1、R2的值
        STR     R1,R6,#0
        ADD     R6,R6,#-1       
        STR     R2,R6,#0
        LDR     R2,R0,#0        ;初始化R2,R3（DFS）（cell data and address of the caller）
        ADD     R3,R0,#0
        JSR     DFS
        LDR     R2,R6,#0        ;返回R2,R1的值
        ADD     R6,R6,#1
        LDR     R1,R6,#0
        ADD     R6,R6,#1
        LDR     R7,R1,#0      
